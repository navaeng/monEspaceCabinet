...
async def display_environnement_entreprise(doc, exp):
    ...
    p_environnement = doc.add_paragraph()
    p_environnement.paragraph_format.keep_together = True
    p_environnement.paragraph_format.keep_with_next = True

    p_environnement.paragraph_format.space_before = Pt(10)
    p_environnement.paragraph_format.space_after = Pt(10)

    run_t = p_environnement.add_run("Environnement")
    run_t.font.size = Pt(11)
    run_t.font.color.rgb = RGBColor(0x00, 0x20, 0x60)
    run_t.bold = True

    run_sep = p_environnement.add_run(" : ")
    run_sep.font.color.rgb = RGBColor(0x00, 0x20, 0x60)
    run_sep.bold = True

    run_desc = p_environnement.add_run(str(secteur_reel))
    run_desc.font.color.rgb = RGBColor(0x00, 0x20, 0x60)