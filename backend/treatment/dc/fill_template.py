...
def fill_template(self):
    QMessageBox.information(
        self,
        "Génération du dossier de compétences",
        "Vous allez choisir où sera généré le dossier de compétences",
    )
    ...
    def worker(selected_file, file_label):
        try:
            ...
            (
                ...
                data_diplomes,
                data_experiences,
            ) = extract_and_call_prompt(selected_file, file_label, self)
            ...
            self.file_label.setText(
                "Tokens gratuit fournis par le cloud atteint pour aujourd'hui"
            )

    threading.Thread(
        target=worker, args=(self.selected_file, self.file_label), daemon=True
    ).start()
